...
function concentrationMultiplier(ratio) {
...
function percentilePoints(value, max, factorMax) {
  if (value <= 0 || max <= 0) return 0;
  return Math.min(factorMax, (value / max) * factorMax);
}

function invertedPercentilePoints(value, max, factorMax) {
  if (max <= 0) return factorMax;
  return Math.max(0, (1 - value / max) * factorMax);
}

function isStrictPositiveMonth(m) {
...
function scoreStockInCategory(raw, maxes) {
