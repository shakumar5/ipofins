/** Strip AMFI scheme-code prefixes (e.g. ib31-) from fund slugs. */
export function stripAmfiSchemePrefixFromSlug(slug) {
  return String(slug || '').replace(/^[a-z0-9]{2,6}-/i, '');
}
...
export function isDuplicateFundPair(slugA, slugB, nameA, nameB, aliases = {}) {
  ...
  if (stripAmfiSchemePrefixFromSlug(slugA) === stripAmfiSchemePrefixFromSlug(slugB)) {
    return true;
  }
