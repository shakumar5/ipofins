    ...
    <div className="space-y-8">
      ...
      <div className="space-y-6">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <label className="text-sm font-medium text-surface-700 dark:text-surface-300">Monthly Investment</label>
            <span className="text-sm font-bold text-surface-900 dark:text-white">₹{monthlyInvestment.toLocaleString('en-IN')}</span>
          </div>
          ...
          <div className="flex justify-between text-xs text-surface-500 mt-1"><span>₹500</span><span>₹1,00,000</span></div>
        </div>

        {/* Annual Step-Up */}
        <div>
          <div className="flex items-center justify-between mb-2">
            <label className="text-sm font-medium text-surface-700 dark:text-surface-300">Annual Step-Up</label>
            <span className="text-sm font-bold text-surface-900 dark:text-white">{annualStepUp}%</span>
          </div>
          <input type="range" min="0" max="30" step="1" value={annualStepUp}
            onChange={(e) => setAnnualStepUp(Number(e.target.value))}
            className="w-full h-2 bg-surface-200 dark:bg-surface-700 rounded-lg appearance-none cursor-pointer accent-primary-600" />
          <div className="flex justify-between text-xs text-surface-500 mt-1"><span>0%</span><span>30%</span></div>
        </div>

        {/* Expected Return */}
        <div>
          <div className="flex items-center justify-between mb-2">
            <label className="text-sm font-medium text-surface-700 dark:text-surface-300">Expected Annual Return</label>
          ...
          <input type="range" min="1" max="30" step="0.5" value={expectedReturn}
            ...
            className="w-full h-2 bg-surface-200 dark:bg-surface-700 rounded-lg appearance-none cursor-pointer accent-primary-600" />
          <div className="flex justify-between text-xs text-surface-500 mt-1"><span>1%</span><span>30%</span></div>
        </div>

        {/* Time Period */}
        <div>
          <div className="flex items-center justify-between mb-2">
            <label className="text-sm font-medium text-surface-700 dark:text-surface-300">Time Period</label>
            <span className="text-sm font-bold text-surface-900 dark:text-white">{timePeriod} years</span>
          </div>
          <input type="range" min="1" max="40" step="1" value={timePeriod}
            onChange={(e) => setTimePeriod(Number(e.target.value))}
            className="w-full h-2 bg-surface-200 dark:bg-surface-700 rounded-lg appearance-none cursor-pointer accent-primary-600" />
          <div className="flex justify-between text-xs text-surface-500 mt-1"><span>1 year</span><span>40 years</span></div>
        </div>
      </div>
      ...
      <div className="bg-surface-50 dark:bg-surface-800/50 rounded-xl p-6 space-y-4">
        ...
        <div className="flex items-center gap-4 text-xs">
          <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded bg-blue-500" /><span className="text-surface-600 dark:text-surface-400">Invested</span></div>
          <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded bg-green-500" /><span className="text-surface-600 dark:text-surface-400">Returns</span></div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 pt-4 border-t border-surface-200 dark:border-surface-700">
          <div className="text-center">
            ...
            <p className="text-lg font-bold text-surface-900 dark:text-white mt-1">{formatCurrency(result.invested)}</p>
          </div>
          <div className="text-center">
        ...
        <div className="mt-4 pt-4 border-t border-surface-200 dark:border-surface-700">
          <p className="text-xs text-surface-500 dark:text-surface-400 mb-2">Step-Up SIP vs Regular SIP Comparison</p>
          <div className="grid grid-cols-2 gap-4">
            <div className="text-center bg-white dark:bg-surface-900 rounded-lg p-3">
              ...
              <p className="text-sm font-bold text-surface-700 dark:text-surface-300 mt-1">{formatCurrency(result.normalValue)}</p>
            </div>
