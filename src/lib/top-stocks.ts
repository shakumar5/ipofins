/**
 * Top Stocks - net rupee accumulation / distribution by source and stock market cap.
 * Loaded from Neon at build time only (no export JSON, no changes to Smart Money).
 */
...
function bucketKey(source: TopStocksSource, cap: TopStocksCap, flow: TopStocksFlow): string {
...
function buildBuckets(source: TopStocksSource, rows: RawFlowRow[]): Record<string, TopStockRow[]> {
